...
// Métodos públicos para uso pelo coordinator
func (s *transactionService) PrepareTransaction(transaction *domain.Transaction) error {
	usdbrlRate, err := s.currencyService.GetUSDToBRL(transaction.OperationDate)
	if err != nil {
		return err
	}
...
// Métodos auxiliares - apenas para transações
